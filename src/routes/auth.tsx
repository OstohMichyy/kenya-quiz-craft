...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Sign in or create a StudyForge account to generate curriculum-aligned quizzes." },
    ],
  }),
  component: AuthPage,
});
...
function AuthPage() {
  ...
  async function google() {
    ...
    if (res.error) setErr(res.error instanceof Error ? res.error.message : String(res.error));
  }
      ...
      <div className="mx-auto flex max-w-md flex-col px-4 py-10">
        <div className="rounded-2xl border border-border/60 bg-gradient-card p-6 shadow-soft">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {mode === "login"
              ? "Sign in to generate and save quizzes."
              : "Get 1 free quiz, then KES 100/month for unlimited."}
          </p>

          <button
            onClick={google}
            type="button"
            className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-lg border border-border bg-background px-4 py-2.5 text-sm font-semibold hover:bg-accent"
          >
          ...
          <form onSubmit={submit} className="space-y-3">
            {mode === "signup" && (
              <input value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Full name"
                className="w-full rounded-lg border border-border bg-background px-3 py-2.5 text-sm" required />
            )}
            <div className="relative">
              <Mail className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" />
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email"
                className="w-full rounded-lg border border-border bg-background pl-9 pr-3 py-2.5 text-sm" required />
            </div>
            <div className="relative">
          ...
          <p className="mt-4 text-center text-xs text-muted-foreground">
            {mode === "login" ? "New to StudyForge?" : "Already have an account?"}{" "}
